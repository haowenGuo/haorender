...
public:
	...
	int channels;
	MyColor() : val(0), channels(0) {}
	MyColor(const unsigned char* p, int channels0) : val(0), channels(channels0) {
		for (int i = 0; i < channels; i++) {
			raw[i] = p[i];
		}
	}
	MyColor(unsigned char b0, unsigned char g0, unsigned char  r0, unsigned char  a0, int channels0):r(r0),g(g0),b(b0),a(a0),channels(channels0) {}
...
public:
	...
	MyImage(const MyImage& p) {
		if (this != &p) {
			...
			if (nbytes > 0) {
				...
				p.data ? memcpy(data, p.data, nbytes) : memset(data, 0, nbytes);
			}
		}
	};
	MyImage(const MatrixXd& p) {
		delete[] data;
		width = static_cast<int>(p.cols());
		height = static_cast<int>(p.rows());
		channels = 1;
		unsigned long nbytes = width * height * channels;
		try {
			data = new unsigned char[nbytes];
	...
	MyImage(unsigned char* data0, int width0, int height0, int channels0) {
		...
		if (nbytes > 0) {
			data = new unsigned char[nbytes];
			data0 ? memcpy(data, data0, nbytes) : memset(data, 0, nbytes);
		}
	}
	MyImage(const string& path) {}

	MyColor get(int x, int y) const {
	...
	MyImage& operator =(const MyImage& img) {
		if (this != &img) {
			...
			if (nbytes > 0) {
				data = new unsigned char[nbytes];
				img.data ? memcpy(data, img.data, nbytes) : memset(data, 0, nbytes);
			}
		}
		return *this;
	};
	void operator =(MatrixXd& img) {
		if (data) delete[] data;
		width = static_cast<int>(img.cols());
		height = static_cast<int>(img.rows());
		channels = 1;
		unsigned long nbytes = width * height * channels;
		try {
			data = new unsigned char[nbytes];
...
struct vertex {
	...
	vertex(Vector4f v0, Vector4f n0,vector<Vector2f> textures0) {
		v = v0;
		n = n0;
		t = Vector4f(0.f, 0.f, 0.f, 0.f);
		b = Vector4f(0.f, 0.f, 0.f, 0.f);
		textures = textures0;
	}
};
struct texture {
